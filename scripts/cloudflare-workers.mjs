...
function buildWorker() {
  ...
  writeFileSync(
    ...
    [
      ...
      "const TEST_HOSTNAME = \"test.pkspot.app\";",
      "function isTestHostname(hostname) {",
      "  return hostname === TEST_HOSTNAME || hostname.endsWith(`.${TEST_HOSTNAME}`);",
      "}",
      "export default {",
      "  async fetch(request) {",
      '    const response = (await handler(request)) ?? new Response("Not Found", { status: 404 });',
      "    if (!isTestHostname(new URL(request.url).hostname)) return response;",
