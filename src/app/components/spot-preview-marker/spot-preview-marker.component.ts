...
@Component({
  ...
  styles: `
    ...
    .spot-preview-marker {
      --spot-marker-collision-height: 52px;
      --spot-marker-collision-width: 124px;
      position: relative;
      display: block;
      width: var(--spot-marker-collision-width);
      height: var(--spot-marker-collision-height);
      cursor: pointer;
      outline: none;
      overflow: visible;
      pointer-events: none;
...
export class SpotPreviewMarkerComponent {
  ...
  private getEnvironmentIcons(amenities: AmenitiesMap | undefined): string[] {
    ...
    if (amenities.indoor === true) {
      ...
      icons.push(AmenityIcons.covered ?? "umbrella");
    }
    return icons;
  }
}
