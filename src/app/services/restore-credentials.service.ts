...
// Disabled until server revocation, sign-in races, and device-transfer checks pass.
export const RESTORE_CREDENTIALS_ENABLED = new InjectionToken<boolean>(
  "RESTORE_CREDENTIALS_ENABLED",
...
export class RestoreCredentialsService {
  private readonly enabled = inject(RESTORE_CREDENTIALS_ENABLED);
  ...
  async restoreSignedOutSession(): Promise<void> {
    if (!this.enabled || !this.isSupported() || this.restoreAttempted) return;
