...
/**
 * Resolves a resized URL only when a stored profile-picture reference exists.
 * Unlike deriving a URL from a user ID, this does not request missing objects
 * for users who never uploaded a profile picture.
 */
export function getResizedProfilePictureUrl(
  ...
  try {
    return new StorageImage(profilePicture).getSrc(size);
  } catch {
